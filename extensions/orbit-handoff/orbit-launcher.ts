...
export function findOrbit(deps: FindOrbitDeps = realDeps()): string | null {
  // ORBIT_BIN is the name existing users already set; the desktop-specific
  // name wins so it can't be confused with the CLI's own binary.
  const override = deps.env.ORBIT_DESKTOP_BIN || deps.env.ORBIT_BIN;
  if (override) {
