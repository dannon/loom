...
function serverMismatchMessage(boundUrl: string, connectedUrl: string): string {
  return (
    `Notebook is bound to a Galaxy page on ${boundUrl}, ` +
    `but you are connected to ${connectedUrl}. Use /connect to switch first.`
  );
}
  ...
  return withNotebookLock(nbPath, async () => {
    ...
    if (existing) {
      if (existing.galaxyServerUrl !== config.url) {
        throw new Error(serverMismatchMessage(existing.galaxyServerUrl, config.url));
      }
...
export async function pushNotebookToGalaxy(opts: PushOptions = {}): Promise<PushResult> {
  ...
  return withNotebookLock(nbPath, async () => {
    ...
    if (existing) {
      if (existing.galaxyServerUrl !== config.url) {
        throw new Error(
          serverMismatchMessage(existing.galaxyServerUrl, config.url) +
            " Or use notebook_link_galaxy_page to re-link to a page on the connected server.",
        );
      }
