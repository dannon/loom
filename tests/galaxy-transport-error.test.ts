...
describe("transportNudgeDecision", () => {
  it("fires the nudge once on the first transport error and disarms", () => {
    const d = transportNudgeDecision(true, "galaxy_get_histories", "Failed to call tool: Not connected");
  ...
  it("suppresses repeat nudges while disarmed (no spam during the retry loop)", () => {
    const d = transportNudgeDecision(false, "galaxy_get_histories", "Failed to call tool: Not connected");
  ...
  it("leaves state untouched for unrelated (non-galaxy) results", () => {
    expect(transportNudgeDecision(true, "bash", "anything")).toEqual({ showNudge: false, armed: true });
    expect(transportNudgeDecision(false, "read", "anything")).toEqual({ showNudge: false, armed: false });
  });
